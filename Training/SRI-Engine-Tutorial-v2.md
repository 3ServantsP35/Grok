...
┌─────────────────────────────────▼───────────────────────────┐
│  LAYER 0.75: FORCE FIELD ENGINE (NEW — 2026-03-08)          │
│  MSTR-specific credit/liquidity composite force             │
│  Inputs: STRF/LQD, MSTR/IBIT ratio, STRC, STABLE.D, MSTR   │
│  Output: F_net score, zone (5 levels), struct gap flag      │
│  Function: Directional gate for AB2 entries; BOUNCE EXHAUST │
│            signal suppresses false bull near resistance     │
└─────────────────────────────────┬───────────────────────────┘
                                  │ directional force + gate state
┌─────────────────────────────────▼───────────────────────────┐
...
│  AB1: Theta income (credit spreads, CRS-gated, 1-30d)       │
│  AB2: Directional conviction (delta-first, 30d+, ≤10% cap)  │
│  AB3: LEAP accumulation (strategic, cycle-length, ~50%)     │
└─────────────────────────────────┬───────────────────────────┘
                                  │ signals + context
┌─────────────────────────────────▼───────────────────────────┐
...
## 6c. Force Field Engine — Layer 0.75

> **Added 2026-03-08. Python engine: `scripts/mstr_suite_engine.py`. Pine indicator: `pine/MSTR_Suite_Force_Field.pine`.**

### 6c.1 Purpose and Role

The Force Field is an MSTR-specific composite force model that sits between the Howell Phase Engine (Layer 0.5) and the Regime Engine (Layer 1). It measures the five external forces that most directly drive MSTR's near-to-medium term price: Saylor credit quality, MSTR's alpha vs. Bitcoin ETFs, preferred stock stress, stablecoin capital flow, and MSTR's own structural momentum.

Unlike the GLI or Howell engines (which operate at the macro level), the Force Field is purely MSTR-specific. A strong global liquidity environment can coexist with a weak Force Field if Saylor's credit is deteriorating relative to IG bonds.

**Architecture position:** Layer 0.75 — inputs into AB2 directional entry gates and the morning brief Section 4.
...
f_primary  = STRF/LQD LT direction × ST magnitude   (primary force; highest predictive power)
multiplier = f(MSTR/IBIT LT state, 0.4–1.5×)        (amplifies aligned forces, dampens opposed)
f_credit   = STRC ST normalized force × 0.5          (credit floor signal)
f_stable   = STABLE.D LT normalized force × 0.4      (inverted — rising dominance = bearish)
f_self     = MSTR ST normalized force × 0.3          (self-reference gate)
```

**Normalized force formula (per component):**
```
gap     = (fast_TL - slow_TL) / |slow_TL|
gap_vel = gap - gap[1]
mag     = min(|gap| × 40, 1.0)           -- saturates at gap ≥ 0.025
dir     = sign(gap)
decel   = 0.6 if (dir × gap_vel < 0)     -- retreating force discounted
f_norm  = dir × mag × decel
```

**Trackline method by instrument:**

| Instrument | Method | Timeframes |
|---|---|---|
...
| STRF/LQD ratio | EMA20 fast / SMA50 slow on ratio | LT: EMA20/SMA50; ST: EMA5/SMA50 |
| MSTR/IBIT ratio | EMA20 fast / SMA50 slow on ratio | LT: EMA20/SMA50 |

### 6c.3 Signal Zones

Backtested on N=575 bars, Jan 2025 → Mar 2026:

| Zone | F_net Range | Bull WR +10d | Bull WR +20d | Confidence |
|---|---|---|---|---|
| STRONG BULL | F > −0.19 | 42% | 36% | ⚠ PROVISIONAL |
| MOD BULL | F > −0.60 | 46% | 46% | ⚠ PROVISIONAL |
| NEUTRAL | −1.15 to −0.60 | 42% | 29% | Monitoring only |
| MOD BEAR | F < −1.15 | **25%** | **18%** | ✓ HIGH (75% bearish at +10d, N=36) |
| STRONG BEAR | F < −1.66 | 42% | 33% | ✓ HIGH (67% bearish at +20d, N=29) |

> **Confidence asymmetry:** Bear zones are validated at HIGH confidence. Bull zones are PROVISIONAL — the 2025–2026 backtesting period was predominantly bearish, generating insufficient confirmed bullish data. 6-month checkpoint: 2026-09-08. Until then, bull zones are monitoring signals only, not entry triggers.

### 6c.4 Structural Gap Gate — BOUNCE EXHAUST

**Critical finding from 575-bar backtest:**

| Condition | N | Bull WR +10d | Bull WR +20d |
|---|---|---|---|
| STRONG_BULL + price within 10% of LT Slow TL | 211 | 45% | 42% |
| **STRONG_BULL + price >10% below LT Slow TL** | **42** | **21%** | **7%** |
| MOD_BULL + price >10% below LT Slow TL | 66 | 52% | 56% |

When STRONG_BULL fires with price more than 10% below the LT Slow TL, the zone is reclassified as **BOUNCE EXHAUST**. This represents credit metrics temporarily improving on a bear-market bounce before structural resistance at the TL is reclaimed. **93% bearish at +20d.**

When MOD_BULL fires with the same gap, it is classified **EARLY RECOV** — credit is beginning to build from a base, mildly bullish at +20/+30d.

**Rule:** Never act on STRONG_BULL as a bullish signal without first checking the struct gate. If BOUNCE EXHAUST is active, treat as a monitoring/caution signal, not a buy trigger.

### 6c.5 MSTR/IBIT LT Gate

Before acting on any bull zone signal:
1. Check the MSTR/IBIT LT state in the Force Field info table
2. If state = `BULL` or `DECEL+` → gate met, proceed
3. If state = `TRANSIT` or `BEAR` → gate not met, do not enter bullish trades

### 6c.6 AB2 Routing

| Force Field Signal | AB2 Action | Structure |
|---|---|---|
| BOUNCE EXHAUST (STRONG_BULL + struct gap) | Directional AB2 trade | Long LEAP puts or bear call spread |
| MOD_BEAR or STRONG_BEAR, high conviction | Directional AB2 trade | Long LEAP puts or bear call spread |
| EARLY RECOV + CT1/CT2 pre-breakout | Directional AB2 trade | Long LEAP call 60–120 DTE |
| STRONG BULL (no struct gap) + gate met | AB2 monitoring — await structural confirmation | — |

### 6c.7 6-Month Checkpoint (2026-09-08)
...
# If WR >= 70%, graduate bull zones to HIGH confidence:
sqlite3 /root/mstr-engine/data/mstr.db   "INSERT OR REPLACE INTO engine_config VALUES ('suite_bull_graduated','1');"
```
...
## 9. AB1 — Theta Income Engine

> **Framework v4.0 (approved 2026-03-08, Gavin).** AB1 is now exclusively a theta-income bucket. Pre-breakout LEAPs (the former AB1) are reclassified as AB2 directional entries. Historical pre-breakout performance data is preserved in Section 9.7 for reference; that signal logic now routes to AB2.

### 9.1 Philosophy

AB1 harvests time decay (theta) consistently through higher-volume, short-duration option trades. While AB3 holds 2-year LEAPs through the full cycle and AB2 fires occasionally on directional conviction, AB1 runs continuously — generating quarterly income through defined-risk credit spreads and PMCC short legs.

**Primary metrics:**
- Income target: beat STRC hurdle (0.83%/month) after losses
- Volume: higher frequency than AB2/AB3 — multiple trades per quarter
- Risk: small, defined, per-trade ceiling
...
```
G1: CRS SCORE ≥ 7   (Call Ripeness Score — measures income environment quality)
    Source: AB2 CRS Pine indicator or Python CRS engine
    Purpose: Confirms premium, volatility, and timing alignment for income trades

G2: LOI IN INCOME WINDOW
    S1_CHOP:  -45 to -20  (primary income window — highest CRS reliability)
    RECOVERY: -20 to 0    (secondary window — wider strikes required)
    NOT in: deep accumulation (LOI < -45) — preserve upside; distribution (LOI > 0) — gate opens for AB2

G3: EXERCISE RISK LOW or MED
    VST SRI Bias ≤ 0: LOW exercise risk
    VST SRI Bias 1–5: MED exercise risk
    VST SRI Bias > 5: HIGH — pause AB1, risk of strike being tested
```

### 9.3 Trade Structure

**Holding period:** 1-day to 2-week target; 30 DTE hard maximum.
**Capital:** ≤10% of total portfolio (excludes AB3 LEAP long leg in any PMCC).

**Permitted structures (defined-risk only — no naked positions):**

| Structure | When | DTE | Notes |
|---|---|---|---|
| Bull put spread | LOI in S1_CHOP or RECOVERY, bullish lean | 7–30 | Standard income vehicle |
| Bear call spread | LOI in RECOVERY, elevated IV, bearish lean | 7–30 | Sell into bounces |
| PMCC short leg (theta) | AB3 LEAP held, CRS ≥7, theta-motivated | Any (default AB1) | See PMCC classification rules |

**PMCC classification note:** The PMCC short call leg is AB1 when theta is the primary motivation (routine income cycle, CRS-guided). It is AB2 when directional conviction drives the decision (larger size, 30+ DTE, specific view). Document intent at entry.

**Edge case:** A 45 DTE short call opened against an AB3 LEAP with CRS ≥7 in S1 CHOP is classified AB1 even if it exceeds 30 DTE. Intent = theta income. Note the exception in the trade log.

### 9.4 Strike Selection (AB1 Income Mode)

| LOI Stage | Strike Width | Delta Target |
|---|---|---|
| S1_CHOP (-45 to -20) | +15% OTM minimum | δ ≤ 0.25 |
| RECOVERY (-20 to 0) | +17–20% OTM | δ ≤ 0.20 |

**Rule:** Never sell calls within 15% of current spot in income mode (protects against gap moves).

### 9.5 Exit Rules

| Trigger | Action |
|---|---|
| Premium reaches 90% decay | Close for full profit — no need to wait for expiry |
| 30-day time stop | Close regardless of P&L |
| Exercise Risk flips HIGH (VST > 5) | Close immediately — do not hold through |
| LOI drops below -45 | Close and pause AB1 — preserve upside window |
| Force Field enters STRONG BEAR | Evaluate: close bearish spreads; pause new trades |

### 9.6 AB1 vs AB2 Decision Tree

```
Is this a SHORT-DURATION income trade (CRS ≥7, theta-motivated)?  → AB1
Is this a DIRECTIONAL conviction trade (Force Field signal, pre-breakout)? → AB2
Is the holding period expected to be 30+ days?                             → AB2
Is this a PMCC short leg opened for routine income?                        → AB1 (default)
Is this a PMCC short leg opened because of a specific directional view?    → AB2 (note at entry)
```

### 9.7 Legacy Pre-Breakout Performance (Historical Reference)

The following data is from the former AB1 pre-breakout LEAP engine (now reclassified as AB2 directional). These win rates inform the AB2 signal quality for CT1/CT2 pre-breakout entries:
...
**Key finding:** The move does happen — the variable is timing. The entry signal (CT1/CT2 pre-breakout) now triggers AB2 directional LEAP calls (60–120 DTE) rather than short tactical LEAPs (90-day max).

---

## 10. AB2 — Directional Conviction Engine

> **Framework v4.0 (approved 2026-03-08, Gavin).** AB2 is now a directional conviction bucket — delta-first, event-driven, episodic. The former AB2 PMCC income overlay is reclassified to AB1 (theta income). Historical PMCC gate data and entry-timing performance is preserved below for reference.

> **Permanent rule:** NO NAKED SHORTS. All short option legs must be part of a defined-risk spread. No exceptions. This rule cannot be overridden by any approval.

### 10.1 Philosophy

AB2 fires when the engine has high confidence in a directional move and the Force Field (Layer 0.75) or structural indicators provide clear confirmation. It is not a weekly activity — it may fire only once per quarter. But when it fires, it deploys meaningful capital (up to 10% of portfolio) in a simple, high-conviction structure.

**Primary distinction from AB1:**
- **AB1:** Theta-first. Sell time. Frequent, small, CRS-gated.
- **AB2:** Delta-first. Buy direction. Episodic, meaningful, Force Field-gated.

### 10.2 Entry Triggers

At least one high-confidence directional trigger required:

| Trigger | Direction | Evidence Required |
|---|---|---|
| Force Field BOUNCE EXHAUST | Bearish | F_net STRONG_BULL + price >10% below LT Slow TL (93% bearish +20d) |
| Force Field MOD_BEAR or STRONG_BEAR, confirmed | Bearish | Force Field zone + AB3 LOI context aligned |
| CT1/CT2 pre-breakout + Force Field EARLY RECOV | Bullish | Both conditions required; MSTR/IBIT LT gate must be met |
| AB3 LOI extreme (< -80) at confirmed Stage 2 | Bullish | LOI deep accumulation; use AB2 LEAP call rather than waiting for AB3 |

### 10.3 Permitted Structures (Simple Only — Max 2 Legs)

**No naked positions under any circumstances.**

| Structure | Direction | Holding Period | Notes |
|---|---|---|---|
| Long LEAP puts | Bearish | 90+ days typical | Defined risk (max loss = premium); high conviction only |
| Bear call spread | Bearish | 30–90 DTE | Wide strikes (+15% OTM min); simple, defined risk |
| Bull put spread | Bullish | 30–90 DTE | High confidence signal required |
| Long LEAP calls (60–120 DTE) | Bullish | 60–120 days | CT1/CT2 pre-breakout + Force Field EARLY RECOV gate |

### 10.4 Capital Rules

- **≤10% of total portfolio at any time**
- **AB1 + AB2 combined ≤ 20% of total portfolio**
- AB2 uses standalone capital — independent of AB3 LEAP positions
- Exception: PMCC short leg classified as AB2 (directional) uses no additional capital (AB3 LEAP is the long leg)

### 10.5 IV Regime Overlay

| IV Percentile | AB2 Action |
|---|---|
| 90th+ (Ultra-High) | Ideal for long LEAP puts — elevated premium; buy before IV collapses |
| 70–90th (High) | Standard AB2 execution |
| 30–70th (Normal) | Proceed if directional signal is strong |
| <30th (Low) | Avoid long options (cheap for a reason); spreads only |

### 10.6 Exit Rules

| Trigger | Action |
|---|---|
| Thesis confirmed (50%+ of expected move) | Take partial profit (50%); trail remainder |
| Kill condition triggered | Close immediately — see hypothesis block at entry |
| Force Field zone reverses against position | Evaluate immediately; close if force flips |
| 90-day time stop | Close regardless |

### 10.7 PMCC Short Leg — AB2 Classification

When a PMCC short call leg is classified as AB2 (directional conviction):
- Opened because of a specific directional view (not routine income)
- Duration ≥ 30 DTE or size larger than standard income cycle
- **Document at entry:** why this is AB2, not AB1; what the directional thesis is
- Exit rules follow AB2 (thesis-based, not theta-decay-based)

### 10.8 Validated Entry-Timing Performance (Legacy AB2 — Income Context)

The following data is from the former AB2 PMCC income engine (now AB1 theta income). These win rates still apply to the LOI/CT entry gates that gate AB1 income timing:
...
**PMCC income window availability** (% of bars in income gate — now AB1 context):
...
| Version | AB2 Definition | Status |
|---|---|---|
| v1.0 | Bull Put Spreads (any context) | Retired |
| v2.0 | Bull Put Spreads (MIXED context only) | Retired |
| v3.0 | PMCC income overlay on AB3 LEAPs | Retired → reclassified to AB1 |
| **v4.0** | **Directional conviction — delta-first, ≤10% capital** | **Active** |
...
### 13.1 Baseline Allocation (v4.0)

> **Major change from v3.0 (2026-03-08):** AB1 and AB2 are completely redefined. AB1 is now theta income (≤10%); AB2 is directional conviction (≤10%). No naked shorts under any circumstances.

| Bucket | Purpose | Baseline | Floor | Ceiling |
|---|---|---|---|---|
| AB3 | 2-year LEAP accumulation | **~50%** | None | 35% (soft alert) |
| AB2 | Directional conviction | **≤10%** | — | 10% (hard cap) |
| AB1 | Theta income | **≤10%** | — | 10% (hard cap) |
| AB4 | Yield-bearing cash reserve | **≥10%** (floor); balance of undeployed | **10% true cash** | 100% |

**Combined rule:** AB1 + AB2 ≤ 20% of total portfolio at all times.

**In structural bear market:** AB4 typically runs 30–40%. Expect: AB3 25–35%, AB1 5%, AB2 5%, AB4 55–65%.

**Permanent rule — no exceptions:** No naked short positions. Every short leg has a defined long hedge at entry.
...
### 13.8 Capital Deployment Priority (v4.0)

1. **AB3 Stage 2 bounce** — Highest conviction; full sizing (~50% target)
2. **AB2 Directional** — Force Field BOUNCE EXHAUST or CT pre-breakout + EARLY RECOV (≤10%)
3. **AB3 Stage 1 watch** — Begin reducing AB4 gradually; await Stage 2 confirmation
4. **AB1 Theta income** — CRS ≥7, S1 CHOP/RECOVERY, ExRisk LOW/MED (≤10%)
5. **AB4** — Default. All undeployed capital. Minimum 10% true cash always.

### 13.9 Portfolio Independence
...
0.75. **Layer 0.75 (Force Field)** — check F_net zone and struct gate. BOUNCE EXHAUST = potential AB2 bearish entry. STRONG/MOD BEAR = bearish headwind (reduce sizing confidence on new longs). Bull zones PROVISIONAL until 2026-09-08 checkpoint; require MSTR/IBIT LT gate before acting.
...
4. **AB2 directional** — Force Field BOUNCE EXHAUST or CT1/CT2 + Force Field EARLY RECOV; ≤10% capital
5. **AB3 accumulation (LOI < acc_thresh)** — standard strategic entry
6. **AB1 theta income** — CRS ≥7, income window open, ExRisk LOW/MED; ≤10% capital
7. **AB3 trim signals** — reduce existing positions
8. **AB1/AB2 exit** — close on thesis completion, time stop, or kill condition
...
### 16.3 The AB2/AB3 Sequencing Pattern (Pre-Breakout)

The ideal pattern is:
```
1. AB3 fires LOI < -60 (deep accumulation) → build AB3 strategic position
2. LOI recovers, Stage 4→1 fires + Force Field EARLY RECOV → AB2 pre-breakout (LEAP call 60-120 DTE)
3. AB2 LEAP captures 10-40% move in 60-120 days (exit on thesis completion or LT+)
4. AB3 position continues holding through full cycle
5. LOI reaches +60-80 → AB3 begins phased trims
6. Parallel: AB1 theta income runs throughout on CRS ≥7 windows
```

AB2 and AB3 are complementary. AB2 captures the breakout swing; AB3 holds through the full cycle. AB1 generates income throughout regardless of where we are in the cycle.
